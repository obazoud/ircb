...
var IRCb = function (options, cb) {
  ...
  self.serverPassword = options.serverPassword;
  ...
  self.connection = (self.secure ? tls : net).connect({
    ...
    async.series([
      function (next) {
        return options.serverPassword
          ? self.pass_(options.serverPassword, next)
          : next();
      },
      function (next) {
        return options.nick
      ...
      function (next) {
        return options.password
          ? self.say('nickserv', 'IDENTIFY ' + options.password, next)
          : next();
      },
      function (next) {
...
IRCb.prototype.pass_ = function (nick, cb) {
  this.write('PASS ' + nick, cb);
};
